    ...
    if 'user_job' not in tables:
        ...
        db_pool.execute('''
            CREATE TABLE user_job (
                ...
                viewed BOOLEAN NOT NULL DEFAULT 0 COMMENT '是否查看过该职位',
                sent_cv BOOLEAN NOT NULL DEFAULT 0 COMMENT '是否发送过简历',
                filter_hash VARCHAR(50) NOT NULL COMMENT '过滤条件的hash值',
                UNIQUE KEY unique_user_job (user_id, job_id, filter_hash),
                PRIMARY KEY (id),
    ...
    if 'user_rule' not in tables:
        ...
        db_pool.execute('''
            CREATE TABLE user_rule (
                id INT NOT NULL AUTO_INCREMENT,
                rule_name VARCHAR(50) NOT NULL UNIQUE COMMENT '规则名称',
                rule_description TEXT COMMENT '规则描述',
                user_id INT NOT NULL COMMENT '用户id',
                keyword VARCHAR(300) COMMENT '关键词',
                city VARCHAR(300) COMMENT '城市列表',
                degree VARCHAR(300) COMMENT '学历条件列表',
                industry VARCHAR(300) COMMENT '行业条件列表',
                title VARCHAR(300) COMMENT '职位类型列表',
                experience VARCHAR(300) COMMENT '经验要求列表',
                scale VARCHAR(300) COMMENT '公司规模列表',
                stage VARCHAR(300) COMMENT '融资阶段列表',
                limit_num INTEGER COMMENT '限制数量',
                PRIMARY KEY (id),
                FOREIGN KEY (user_id) REFERENCES user(id)
            )
        ''')
